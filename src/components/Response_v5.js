...
function Response_v5() {
    ...
    const[loadersResponseOk, setLoadersResponseOk] = useState(false);
    // информация для отображения в таблице - "Общая инфо", "ТО" или "Рекламации"
    ...
    async function handleSubmit(event) {
        ...
        console.log('loadersResponseOk:', loadersResponseOk);
        if (!mistakeHTTPinLoaders) {
          const resultSerNumArr = [resultSerNum];
          let resCommon = resultSerNumArr.map(function(item) {
            return <tr key={item.id}>
              <td><Link to={{pathname: "details"}} state={{from: detailLoader}} className="section-main-panel-table-row-link">{item.serialNumber}</Link></td>
              <td><Link to={{pathname: "details"}} state={{from: detailLoader}} className="section-main-panel-table-row-link">{item.modelOfLoader}</Link></td>
              <td><Link to={{pathname: "details"}} state={{from: detailLoader}} className="section-main-panel-table-row-link">{item.modelOfEngine}</Link></td>
    ...
    function handleClickCommon(event) {
        ...
        let resCommon = detailLoader.map(function(item) {
            return <tr key={item.id}>
                <td><Link to={{
                  pathname: "details"}} state={{from: detailLoader}}
                  className="section-main-panel-table-row-link">{item.serialNumber}</Link></td>
                
                <td><Link to={{
                  pathname: "details"}} state={{from: detailLoader}} className="section-main-panel-table-row-link">{item.modelOfLoader}</Link></td>
                <td><Link to={{
                  pathname: "details"}} state={{from: detailLoader}} className="section-main-panel-table-row-link">{item.modelOfEngine}</Link></td>
                <td><Link to={{
                  pathname: "details"}} state={{from: detailLoader}} className="section-main-panel-table-row-link">{item.serialNumberEngine}</Link></td>
                <td><Link to={{
                  pathname: "details"}} state={{from: detailLoader}} className="section-main-panel-table-row-link">{item.modelOfTransmission}</Link></td>
                <td><Link to={{
                  pathname: "details"}} state={{from: detailLoader}} className="section-main-panel-table-row-link">{item.serialNumberTransmission}</Link></td>
                <td><Link to={{
                  pathname: "details"}} state={{from: detailLoader}} className="section-main-panel-table-row-link">{item.modelOfLeadingAxle}</Link></td>
                <td><Link to={{
                  pathname: "details"}} state={{from: detailLoader}} className="section-main-panel-table-row-link">{item.serialNumberLeadingAxle}</Link></td>
                <td><Link to={{
                  pathname: "details"}} state={{from: detailLoader}} className="section-main-panel-table-row-link">{item.modelOfSteerAxle}</Link></td>
                <td><Link to={{
                  pathname: "details"}} state={{from: detailLoader}} className="section-main-panel-table-row-link">{item.serialNumberSteerAxle}</Link></td>
                <td><Link to={{
                  pathname: "details"}} state={{from: detailLoader}} className="section-main-panel-table-row-link">{isLoggedIn && item.supplyContractNumDate}</Link></td>
                <td><Link to={{
                  pathname: "details"}} state={{from: detailLoader}} className="section-main-panel-table-row-link">{isLoggedIn && item.dateOfShippingFactory}</Link></td>
                <td><Link to={{
                  pathname: "details"}} state={{from: detailLoader}} className="section-main-panel-table-row-link">{isLoggedIn && item.recipient}</Link></td>
                <td><Link to={{
                  pathname: "details"}} state={{from: detailLoader}} className="section-main-panel-table-row-link">{isLoggedIn && item.deliveryAddress}</Link></td>
                <td><Link to={{
                  pathname: "details"}} state={{from: detailLoader}} className="section-main-panel-table-row-link">{isLoggedIn && item.equipment}</Link></td>
                <td><Link to={{
                  pathname: "details"}} state={{from: detailLoader}} className="section-main-panel-table-row-link">{isLoggedIn && item.client}</Link></td>
                <td><Link to={{
                  pathname: "details"}} state={{from: detailLoader}} className="section-main-panel-table-row-link">{isLoggedIn && item.serviceCompanyLoader}</Link></td>
            </tr>;
        });
        let headerCommon =  <tr>
            <th>Зав. № машины</th>
    ...
    function handleFruitChange() {
        ...
        console.log('Вы выбрали: ' + selectedFruit);
    }
